...
router.post("/add", upload.fields([{ name: 'orden_compra', maxCount: 1 }, { name: 'num_pedido', maxCount: 1 }, { name: 'comprobante_pago', maxCount: 1 }]), async(req, res) => {
    ...
    if (req.body.nombre_cliente != undefined && req.body.nombre != ' ' && req.body.observaciones.length < 250) {
        ...
        const pedidos = await pool.query(`SELECT orden_de_compra,ruta,estatus,ruta_pdf_orden_compra,ruta_pdf_pedido,ruta_pdf_comprobante_pago ,num_pedido,observacion,DATE_FORMAT(fecha_inicial,'%y-%m-%d %H:%i %p') fecha_inicial,comprobante_pago,importe,detalles
                                        FROM pedidos`);
        res.send(pedidos);
